...
quaternioni operator = (quaternioni b) //uguaglianza
	{for (int p=0; p<4; ++p) {
	this->co[p]=b.co[p];}
	return *this;
}
...
quaternioni operator + (quaternioni  b) //somma
	{quaternioni c;
	for (int p=0; p<4; ++p)
	{c.co[p]=this->co[p]+b.co[p];}
	return c;}

quaternioni operator - (quaternioni  b) //sottrazione
	{quaternioni c;
	for (int p=0; p<4; ++p)
	{c.co[p]=this->co[p]-b.co[p];}
	return c;}

quaternioni operator += (quaternioni  b) //+=
...
quaternioni operator * (quaternioni b) //prodotto
	{quaternioni c;
...
double operator / (quaternioni b) //distanza
...
quaternioni operator * (double g) // * scalare
	{quaternioni c;
	for (int p=0; p<4; ++p)
	{c.co[p]=this->co[p]*g;}
	return c;
...
quaternioni ()=default;
quaternioni(double a, double i , double j, double k) {co[0]=(a), co[1]=(i), co[2]=(j), co[3]=(k);};
quaternioni(double angolo, double *vet) {
	co[0]=cos(angolo/2); 
	double k;
...
quaternioni rot (quaternioni quat) {return (*this)*quat*(--(*this));}
bool ver () {if (abs(~(*this)-1)<=0.01) return true;
else return false;}
   //******************************//
...
{double el [3][3];
double s;
SOtre(quaternioni q): s(1/(~q)*(~q)) {el[0][0]=1-2*s*(q.co[2]*q.co[2]+q.co[3]*q.co[3]),  el[0][1]=2*s*(q.co[1]*q.co[2]-q.co[3]*q.co[0]),       el[0][2]=2*s*(q.co[1]*q.co[3]+q.co[2]*q.co[0]), 
...
vettori()=default;
double co[3];
vettori(quaternioni q){for (int i=0; i<3; ++i) {co[i]=q.co[i+1]/(sin(acos(q.co[0])));}
;}
};
